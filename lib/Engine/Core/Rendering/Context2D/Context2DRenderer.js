...
var Shape_1 = require("../../Collision/Shape/Shape");
...
var Context2DRenderer = /** @class */ (function () {
    ...
    Context2DRenderer.prototype.render = function (camera, renderData) {
        ...
        if (renderData.type === RenderData_1.RenderDataType.Collider) {
            this.renderCollider(renderData);
        }
    };
    Context2DRenderer.prototype.renderImage = function (renderData) {
        ...
        if (renderData.rotation) {
            ...
            this.imagePosition.set(renderData.flipHorizontal ? -renderData.width : this.imagePosition.x, renderData.flipVertical ? -renderData.height : this.imagePosition.y);
        }
    ...
    Context2DRenderer.prototype.renderCollider = function (renderData) {
        var _this = this;
        this.canvasContext.save();
        this.updateRenderPosition(renderData);
        this.canvasContext.beginPath();
        this.canvasContext.strokeStyle = renderData.color;
        var shape = renderData.shape.clone();
        shape.position = renderData.viewportPosition;
        shape.update();
        switch (shape.type) {
            case Shape_1.ShapeType.Rectangle:
                this.canvasContext.moveTo(shape.vertex[0].x, shape.vertex[0].y);
                [1, 2, 3, 0].forEach(function (index) {
                    _this.canvasContext.lineTo(shape.vertex[index].x, shape.vertex[index].y);
                });
        }
        this.canvasContext.strokeStyle = renderData.color;
    ...
    Context2DRenderer.prototype.updateRenderPosition = function (renderData) {
        ...
        renderData.viewportPosition.set(Number((renderData.viewportPosition.x + this.canvas.width / 2).toFixed(0)), Number((this.canvas.height / 2 - renderData.viewportPosition.y).toFixed(0)));
    };
    Context2DRenderer.prototype.centerImage = function (renderData) {
        renderData.viewportPosition.set(renderData.viewportPosition.x - Math.floor(renderData.width / 2), renderData.viewportPosition.y + Math.floor(renderData.height / 2));
    };
