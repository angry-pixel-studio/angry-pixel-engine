...
import { MAX_LIGHTS, ProgramManager } from "../program/ProgramManager";
...
export class DarknessRenderer implements Renderer {
    ...
    public render(renderData: DarknessRenderData, cameraData: CameraData, lastRender?: RenderDataType): boolean {
        ...
        this.gl.uniform1i(this.programManager.numLightsUniform, Math.min(renderData.lights.length, MAX_LIGHTS));

        for (const [i, light] of renderData.lights.entries()) {
            if (i >= MAX_LIGHTS) break;

            const uniforms = this.programManager.lightUniforms[i];
            ...
            this.gl.uniform1f(uniforms.radius, light.radius * cameraData.zoom);
            this.gl.uniform1f(uniforms.smoothMode, light.smooth ? 1 : 0);
            this.gl.uniform1f(uniforms.intensity, Math.max(0, Math.min(1, light.intensity)));
        }
